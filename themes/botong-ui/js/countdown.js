...
$.fn.countdown = function( method /*, options*/ ) {
	var defaults = {
			...
			nextDate: null,
			updateTime: 1000,
			htmlTemplate: "<span title='Day(s)'>%d</span>:<span title='Hour(s)'>%h</span>:<span title='Minute(s)'>%i</span>:<span title='Second(s)'>%s</span>",
			minus: false,
			onChange: null,
			onComplete: null,
		...
		timerFunc = function() {
			...
			if((_replace=$this.attr('tooltip-replacealso'))!='' && settings.replaceTooltip) {
				$(_replace).find('.tooltip-inner').html(time);
				$this.tooltip('update');
				$this.attr('data-original-title',time);
			}
			if(settings.changeHTML) {
				$this.html( time ).trigger("change.jcdevt", [settings] ).trigger("countChange", [settings] );
			}
			else {
				$this.trigger("change.jcdevt", [settings] ).trigger("countChange", [settings] );
			}
